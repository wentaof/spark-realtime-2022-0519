...
import com.atguigu.gmall.util.{BeanUtils_gmall, PropertiesUtils}
...
case class OrderWide(
  ...
  def mergeOrderInfo(orderInfo: OrderInfo) = {
    if(orderInfo != null){
      BeanUtils_gmall.copyProperties(orderInfo,this)
  ...
  def mergeOrderDetail(orderDetail: OrderDetail): Unit = {
    if(orderDetail != null){
      BeanUtils_gmall.copyProperties(orderDetail,this)
